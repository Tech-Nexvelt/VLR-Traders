...
// ============================================================
// MaskedHeading — React Bits (https://reactbits.dev)
// Ported to TypeScript for this project. Animation/DOM logic is
...
export interface MaskedHeadingProps extends Omit<React.HTMLAttributes<HTMLElement>, "style"> {
  text?: string;
...
const MaskedHeading = ({
  ...
  const sync = useCallback(() => {
    ...
    for (let i = 0; i < wordRefs.current.length; i += 1) {
      ...
      glyph.setAttribute("y", `${(base as HTMLElement).offsetTop}`);
